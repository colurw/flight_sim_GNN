...
def fitness_function(plane):
    ...
    score = (plane.frame_count *                                                  # length of flight
            ...
            pow(plane.hits_taken + 1, 0.7))                                       # evasiveness
    return score * 0.0001
...
# combat variables
MAX_FRAMES = 12000           # processor cycles per contest
SAVE_EVERY = 100             # generations
...
for generation in range(GENERATIONS):
    ...
    for model in population: 
        ...
        fitness = fitness_function(current_plane)
    ...
    # save generation statistics and model info to text file
    with open("flight_scores.txt", "a") as file:
    ...
    if winning_score > highest_score:
        ...
        torch.save(highscore_nn.state_dict(), 'highscore_lineage/{}_gen_{}'.format(EPOCH, generation.zfill(4))) 
        highest_score = winning_score

    # save model snapshot periodically
    if (generation + 1) % SAVE_EVERY == 0:
        torch.save(population[index_winner].state_dict(), 'highscore_lineage/{}_gen_{}~'.format(EPOCH, generation + 1))
        print('model saved')
